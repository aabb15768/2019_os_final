...
int kshutdown(ksocket_t socket, int how)
{
	...
	sk = (struct socket *)socket;
	if (sk) ret = sk->ops->shutdown(sk, how);
...
{
	printk("%s version %s\n%s\n%s\n", KSOCKET_NAME, KSOCKET_VERSION, KSOCKET_DESCPT, KSOCKET_AUTHOR);
    return 0;
}
