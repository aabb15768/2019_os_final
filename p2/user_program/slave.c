...
int main (int argc, char* argv[])
{
	...
	switch(method[0])
	{
		...
		case 'm': // mmap I/O
			while((ret = read(dev_fd, buf, sizeof(buf))) > 0){
				...
				if((long)PAGE_SIZE - (long)data_size - (long)ret < 0 || data_size == -1){
					if(data_size != -1){
						...
						if(msync(mmapped, PAGE_SIZE, MS_ASYNC) == -1) perror("Cannot sync to disk\n");
						if(munmap(mmapped, PAGE_SIZE) == -1) perror("Fail to un-map file!\n");
					}

					if(lseek(file_fd, PAGE_SIZE-1, SEEK_END) == -1) perror("Cannot stretch the file\n");
					if(write(file_fd, "", 1) == -1) perror("Cannot resize the file\n");

					mmapped = mmap(NULL, PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, file_fd, page_size);
					if(mmapped == MAP_FAILED) perror("mmap fial!\n");

					data_size = 0;
				}
				
				for(size_t i = 0; i < ret - remain; ++i) mmapped[data_size + i] = buf[remain + i];
			...
			if(msync(mmapped, PAGE_SIZE, MS_ASYNC) == -1) perror("Cannot sync to disk\n");
			if(munmap(mmapped, PAGE_SIZE) == -1) perror("Fail to un-map file!\n");
			if(ftruncate(file_fd, file_size) == -1) perror("Fail to trucate file.\n");
	}
